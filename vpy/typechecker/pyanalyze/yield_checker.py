...
class YieldChecker:
    ...
    variables_from_yield_result: Dict[VersionId, Dict[str, bool]] = field(
    ...
    def record_usage(self, version: VersionId, name: str, node: ast.AST) -> None:
        if (
            version in self.variables_from_yield_result
            and name in self.variables_from_yield_result[version]
        ):
            if self._is_augassign_target(node):
                return
            self.variables_from_yield_result[version][name] = True
