...
class ClassAttributeChecker:
    ...
    def __init__(
        ...
        *,
        ...
        self.attributes_read = collections.defaultdict(
            lambda: collections.defaultdict(list)
        )
        # Dictionary from type to set of attributes that are set on that class
        self.attributes_set = collections.defaultdict(
    ...
    def __exit__(
        ...
        if exc_type is None and self.enabled:
            ...
            if self.should_check_unused_attributes:
                self.check_unused_attributes(env)
    ...
    def record_attribute_read(
        ...
        """Records that attribute `attr_name` was accessed on type `typ` in version `version`."""
        self.filename_to_visitor[visitor.filename] = visitor
        serialized = self.serialize_type(typ)
        if serialized is not None:
            self.attributes_read[serialized][version].append(
                (attr_name, node, visitor.filename)
            )
    ...
    def record_attribute_set(
        ...
        """Records that attribute `attr_name` was set on type `typ` in version `version`."""
        serialized = self.serialize_type(typ)
    ...
    def check_attribute_reads(self, env: "Environment") -> None:
        ...
        """
        for serialized, versions in sorted(
            self.attributes_read.items(), key=self._cls_sort
        ):
            for version, attrs_read in versions.items():
                ...
                for attr_name, node, filename in sorted(
                    ...
                    self._check_attribute_read(
                        ...
                        attr_name,
                        node,
                        self.filename_to_visitor[filename],
                        env,
                    )

    def check_unused_attributes(self, env: "Environment") -> None:
        ...
        def _add_attrs(
            ...
            if typ is None:
                return
            all_attrs_read[typ][version] |= attr_names_read
            for base_cls in typ.__bases__:
                all_attrs_read[base_cls][version] |= attr_names_read
            if isinstance(typ, type):
                for child_cls in qcore.inspection.get_subclass_tree(typ):
                    all_attrs_read[child_cls][version] |= attr_names_read

        for serialized, versions in self.attributes_read.items():
            for version, attrs_read in versions.items():
                attr_names_read = {attr_name for attr_name, _, _ in attrs_read}
                _add_attrs(self.unserialize_type(serialized), version, attr_names_read)

        for typ, attrs in self.options.get_value_for(IgnoredUnusedClassAttributes):
            if typ is not None:
                for version, attrs in all_attrs_read[typ].items():
                    _add_attrs(typ, version, attrs)

        ignored = set(self.options.get_value_for(IgnoredUnusedAttributes))
        for typ, versions in sorted(all_attrs_read.items(), key=self._cls_sort):
            for version, attrs_read in versions:
                ...
                for attr in existing_attrs - attrs_read - ignored:
                    ...
                    if safe_getattr(
                        safe_getattr(typ, attr, None), "server_call", False
                    ):
                        continue
...
class NameCheckVisitor(node_visitor.ReplacingNodeVisitor):
    ...
    def __reduce_ex__(self, proto: object) -> object:
        def _new(cls: type, args, kw):
            "A function to map kwargs into cls.__init__"
            return cls(*args, **kw)

        # Only pickle the attributes needed to get error reporting working
        return _new, (
            self.__class__,
            (self.filename, self.contents, self.tree),
            {"checker": Checker()},
        )
        ...
        *,
        ...
        for decorator in [] if isinstance(node, ast.Lambda) else node.decorator_list:
            ...
            if isinstance(decorator, ast.Call):
                # TODO: Ignore version decorators
    ...
    def visit_FunctionDef(self, node: FunctionDefNode) -> Value:
        # Set current version here. The body of this function is type-checked in
    ...
    def composite_from_name(
        ...
        if force_read or self._is_read_ctx(node.ctx):
            self.yield_checker.record_usage(self.version, node.id, node)
            ...
            if value is not None:
                self.yield_checker.record_assignment(self.version, node.id)
    ...
    def visit_arg(self, node: ast.arg) -> None:
        self.yield_checker.record_assignment(self.version, node.arg)
    ...
    def composite_from_attribute(self, node: ast.Attribute) -> Composite:
        if isinstance(node.value, ast.Name):
            ...
            if self._is_write_ctx(node.ctx):
                self.yield_checker.record_assignment(self.version, attr_str)
            else:
                self.yield_checker.record_usage(self.version, attr_str, node)
            ...
            if isinstance(root_composite.value, TypedValue):
                ...
                if isinstance(typ, type):
                    self._record_type_attr_set(
                        typ, self.version, node.attr, node, self.being_assigned
                    )
    ...
    def get_attribute(
        ...
        *,
        ...
        # TODO: Change here to respect version MRO
        ctx = _AttrContext(
            root_composite,
            attr,
            self,
    ...
    def merge_extra_data(
        ...
        **kwargs: Any,
        ...
        for checker in extra_data:
            if checker is None:
                continue
            for serialized, versions in checker.attributes_read.items():
                for version, attrs in versions.items():
                    attribute_checker.attributes_read[serialized][version] += attrs
